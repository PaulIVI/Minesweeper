...
case class MinesMap(situation: Vector[Vector[Int]]) {
  ...
  def openField(row_and_col_index: Tuple2[Int, Int], mines_map_base: Vector[Vector[Int]]): MinesMap = {
    if (mines_map_base(row_and_col_index._1)(row_and_col_index._2) == 9) {
      copy(Vector.fill(mines_map_base.length)(Vector.fill(mines_map_base.length)(1)))
    } else {
      if (mines_map_base(row_and_col_index._1)(row_and_col_index._2) == 0) {
  ...
  def solve(mines_map_base: MinesMapBase)={
    ...
    // if the player have lost, open all field
    ...
    // if the player won, print '!' in all fields
    else{
